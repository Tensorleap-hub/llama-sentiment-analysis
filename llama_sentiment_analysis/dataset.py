...
def load_data() -> Tuple[Dataset, Dataset, Dataset]:
    ...
    ds_train = load_dataset("glue", "sst2", cache_dir=CONFIG['cache_dir'], split="train").shuffle(seed=SEED)
    ds_val = load_dataset("glue", "sst2", cache_dir=CONFIG['cache_dir'],  split="validation").shuffle(seed=SEED)
    ds_test = load_dataset("glue", "sst2", cache_dir=CONFIG['cache_dir'],  split="test").shuffle(seed=SEED)
